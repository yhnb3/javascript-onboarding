...
6. [문제 6](docs/PROBLEM6.md)
     ...
     - ~~닉네임 비교 함수 구현~~
       ...
       - ~~정규 표현식을 위한 regString 만드는 함수 구현~~
         ...
         - ~~tagetNickname을 정규표현식화 하는 함수~~
           ...
           - 이런 식으로 정규 표현식을 만들때 map을 쓰다보니 마지막이 필요 없는 부분이지만 따로 자르는데 리소스를 소비하기 보다 그냥 똑같은거 하나 더 넣는게 나을거 같다는 생각으로 마지막은 마지막 바로 전 과 같은 것으로 넣었습니다.
   ...
   - 고민했던 부분
     ...
     - email을 정렬할때 도메인을 포함해서 정렬해야하나 id 부분만 따로 떼어내서 정렬해야하나 고민하였습니다.
       - 문제에 email부분으로 정렬하라고 한 것을 보아 도메인을 포함한 부분이라 생각하여 도메인까지 포함한 정렬을 하였습니다.
...
7. [문제 7](docs/PROBLEM7.md)
   ...
   - 요구 기능
     ...
     - ~~친구 점수 객체 만들기~~
       ...
       - user를 제외한 sns 유저들을 key로 가지고 score를 value로 가지는 객체 생성

     - ~~방문한 친구 점수 계산하는 함수 만들기~~

     - ~~친구의 친구인 점수 확인하는 함수 만들기~~

       - ~~친구의 친구인지 체크하는 함수 만들기~~

     - ~~친구 점수 중에 0점보다 높은 친구만 필터링하는 함수 만들기~~

     - ~~점수와 이름으로 sort할때 필요한 콜백함수 만들기~~

     - ~~상위 5명 자르기~~

   - 고민했던 부분

     - `user`의 기존 친구를 제외해야 하는데 어느 부분에서 제외해야 할지 고민이었습니다.
       - 친구 점수를 만들면서 제외하고 싶지만 여기서 제외하더라도 후에 방문한 친구 점수 계산하는 곳에서 추가될 가능성이 있어서 의미 없다고 생각하였습니다.
       - 방문 친구 점수 계산 추가하는 곳에서도 제외하는 것을 고려해보았을때 너무 많은 인자를 포함해야하는 부담이 있었습니다.
       - 기존 친구가 친구 점수 객체에 포함 됨으로써 효율적인 부분을 저해하는 부분은 친구의 친구를 구할때 인데 이 부분에서 기존 친구는 이 과정을 진행하지 않게 예외 처리를 하여 효율적인 부분의 저해를 막았습니다.
       - 그리고 후에 친구 점수가 0이 친구를 제외하는 validFriends 함수에서 점수가 0이거나 기존의 친구인 친구도 filtering 하는 것으로 해결하였습니다.
